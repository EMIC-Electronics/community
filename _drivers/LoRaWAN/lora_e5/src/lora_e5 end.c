...
void loraE5_updateState(char * answer)
{
    ...
    char *aux = answer;
    static uint16_t poll_timeout = 0;
    ...
    switch(lw_state)
    {
        ...
        case LW_LOWPOWER:
            ...
            {
                poll_timeout = timeStamp + 5000;
                status = 1;
            }
            else if (status == 1)
            {
                if (poll_timeout <= timeStamp)
                {
                    status = 0;
...
{
    if (lw_status == LW_LOWPOWER)
    {
        sendCommand("A");

        lw_status = LW_WAIT_FOR_AWAKE;
        loraE5_updateState("");    
    }
}
