void dht11_init()
{
	...
	HAL_GPIO_PinCfg(DTH11_D,GPIO_INPUT);
	...
	HAL_GPIO_PinCfg(DTH11_D, GPIO_OUTPUT);
	//gpio_pinSet(PORT_DTH11_D, GPIO_HIGTH);// lo dejo en estado alto (idle)
	HAL_GPIO_PinSet(DTH11_D, GPIO_HIGH);
...
void dht11_start()
{
	...
	HAL_GPIO_PinCfg(DTH11_D, GPIO_OUTPUT);
	//gpio_pinSet(PORT_DTH11_D, GPIO_HIGTH);// lo dejo en estado alto (idle)
	HAL_GPIO_PinSet(DTH11_D, GPIO_LOW);
		...
		else
		{
			...
			else
			{
				if (timeStamp > DHT11_START_TOUT + dht11_start_tout)
				{
					...
					HAL_GPIO_PinSet(DTH11_D, HPIO_HIGH);
