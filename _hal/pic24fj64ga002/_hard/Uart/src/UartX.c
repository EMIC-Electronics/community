...
unsigned char UARTX_{puerto}__IN_pop(uint8_t extrae)
{
#if miPuerto == 1
	IEC0bits.U1RXIE = 0;
#elif miPuerto == 2
	...
	{
		if (UART_{puerto}__punt_sal_aux == 0xffff)
		{
			UART_{puerto}__punt_sal_aux = UARTX_{puerto}__IN_punt_sal;
		}
		dato = UARTX_{puerto}__IN_fifo[UART_{puerto}__punt_sal_aux];

		UART_{puerto}__punt_sal_aux ++;

		if (dato == UARTX_{puerto}__frame_indentifier)
		{
			UART_{puerto}__punt_sal_aux = 0xffff;// reseteo el valor indicando que ya termino el frame
		}
		if (UART_{puerto}__punt_sal_aux == UARTX_{puerto}__IN_MAX)
		{
			UART_{puerto}__punt_sal_aux = 0;
		}
	}
#if miPuerto == 1
	IEC0bits.U1RXIE = 1;
...
unsigned char UARTX_{puerto}__OUT_pop(void)
{
	...
	if (UARTX_{puerto}__OUT_punt_sal == UARTX_{puerto}__OUT_MAX) UARTX_{puerto}__OUT_punt_sal = 0;
	return dato;
}
